...
print("Type of Elements:", type(subclass_labels_test[0]))
...
if user_input.lower() == 'a':
    ...
    #pr_A2 = calculate_tpr_per_attack(y_test, y_pred.flatten(), subclass_labels_test, 'A2')
    #pr_A4 = calculate_tpr_per_attack(y_test, y_pred.flatten(), subclass_labels_test, 'A4')
    #vg_tpr_SA = (tpr_A2 + tpr_A4) / 2
    #rint(f"Average TPR (SA unknown attacks): {avg_tpr_SA:.4f}")
    ...
    for subclass in ['A2', 'A4']:
        ...
        y_true_sub = np.ravel(y_true_sub)
        y_pred_sub = np.ravel(y_pred_sub)
